...
N = 400  # Número de puntos espaciales
h = L / N  # Espaciado espacial
...
def step(u, N, h, k, delta):
    ...
    for i in range(N):
        ...
        u_new[i] = u[i] - k * (u[i] * du_dx + delta**2 * d3u_dx3)
    return u_new
...
masa_history = []  # Lista para guardar la masa
momento_history = []  # Lista para guardar el momento
energia_history = []  # Lista para guardar la energía
for n in range(M):
    u = step(u, N, h, k, delta)  # Avanzar un paso de tiempo
    if np.any(np.isnan(u)):  # Terminar la simulación cuando el esquema se inestabilice 
        break
    if n % 100 == 0:  # Guardar cada 100 pasos de tiempo
        u_history.append(u.copy())
        masa, momento, energia = conserved_quantities(u, h, delta)
